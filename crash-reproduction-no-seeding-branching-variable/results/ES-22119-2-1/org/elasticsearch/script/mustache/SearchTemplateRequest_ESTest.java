/*
 * This file was automatically generated by EvoSuite
 * Tue Feb 11 07:02:05 CET 2020
 */

package org.elasticsearch.script.mustache;

import org.junit.Test;
import static org.junit.Assert.*;
import static org.evosuite.shaded.org.mockito.Mockito.*;
import static org.evosuite.runtime.EvoAssertions.*;
import org.elasticsearch.action.get.MultiGetShardResponse;
import org.elasticsearch.common.io.stream.StreamInput;
import org.elasticsearch.script.mustache.SearchTemplateRequest;
import org.evosuite.runtime.EvoRunner;
import org.evosuite.runtime.EvoRunnerParameters;
import org.evosuite.runtime.ViolatedAssumptionAnswer;
import org.junit.runner.RunWith;

@RunWith(EvoRunner.class) @EvoRunnerParameters(useVFS = true, useJEE = true) 
public class SearchTemplateRequest_ESTest extends SearchTemplateRequest_ESTest_scaffolding {

  @Test(timeout = 4000)
  public void test0()  throws Throwable  {
      SearchTemplateRequest searchTemplateRequest0 = new SearchTemplateRequest();
      StreamInput streamInput0 = mock(StreamInput.class, new ViolatedAssumptionAnswer());
      doReturn(false, false, false).when(streamInput0).readBoolean();
      doReturn((MultiGetShardResponse) null).when(streamInput0).readOptionalStreamable(nullable(java.util.function.Supplier.class));
      doReturn("").when(streamInput0).readString();
      doReturn(104).when(streamInput0).readVInt();
      // Undeclared exception!
      searchTemplateRequest0.readFrom(streamInput0);
  }
}
